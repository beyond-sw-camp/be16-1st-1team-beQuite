...
<img src="https://github.com/user-attachments/assets/5475642e-191f-41ed-8bbc-091d4c472f48" width="600">
...
-
🤫 김상환
  >이번 야구 데이터베이스 설계 프로젝트에서 저는 주로 데이터 적재와 프로시저 생성을 맡아 진행했습니다. 초기 테이블 스키마가 설계되고 난 이후, 실제 데이터가 제대로 들어가고 조회 가능한 상태로 준비하는 역할을 담당했습니다. 데이터가 없는 설계만으로는 실제 서비스가 어떻게 동작할지 감을 잡기 어렵기 때문에, 샘플 데이터와 실제 CSV 파일을 이용한 데이터 로딩 작업을 수행하여 현실적인 데이터베이스 환경을 구축하는 데 집중했습니다.
또한 데이터를 조회하고 처리하는 주요 기능을 프로시저로 만들어, 팀원들이 기능을 손쉽게 사용할 수 있도록 DB 내부에 비즈니스 로직을 구현했습니다. 이 과정에서 데이터의 정확성, 프로시저의 효율성, 그리고 예외 처리나 트랜잭션을 철저히 관리하는 것이 매우 중요했습니다.
처음 데이터 적재를 시작할 때 가장 고민한 부분은 적재 순서의 정확한 판단이었습니다. 특히 이번 프로젝트의 데이터베이스 구조는 구단(leagueteam) → 선수(players) → 포지션(positioned) → 스탯(batterstats·pitcherstats) → 가상구단(fanteam·fanteammember) 등 여러 테이블이 FK로 긴밀히 연결된 형태였습니다. 처음에는 FK 관계를 깊이 생각하지 않고 직관적으로 데이터를 적재했지만, 이 과정에서 무수히 많은 FK 제약 오류를 만나게 되었습니다.
이러한 시행착오를 통해 외래키가 존재하는 테이블 간의 의존성을 철저히 분석한 뒤, 명확한 데이터 로딩 순서를 계획해야 한다는 점을 깊이 깨달았습니다. 예를 들어, 선수를 먼저 적재하지 않고 선수 포지션 데이터나 스탯 데이터를 먼저 넣으려다 보니 데이터가 적재되지 않는 오류가 발생했고, 이 문제를 해결하기 위해 전체적인 FK 의존 관계를 차트로 정리하여 명확한 적재 순서를 마련했습니다.
또한 대량의 CSV 데이터를 실제로 로딩할 때, 데이터 타입 불일치, 한글 깨짐 현상, 특수 문자 처리 문제 등도 잦은 오류로 나타났습니다. 특히 ENUM 타입으로 관리되는 포지션 데이터를 CSV에서 가져올 때, 사소한 띄어쓰기나 개행 문자로 인해 로딩이 실패하는 일이 빈번히 발생해 많은 시간을 소모하게 되었습니다.
>
-
🤫 김영관
